...
namespace halvoe
{
  ...
  enum class SerializerStatus : uint8_t
  {
    ...
    writeStringOutOfRangeOrNotZeroterminated
  };
    ...
    public:
      static const char* message(SerializerStatus in_code)
      {
        switch (in_code)
        {
          ...
          case SerializerStatus::writeStringOutOfRangeOrNotZeroterminated: return "write string operation out of range or string is not zeroterminated";
  ...
  class Serializer
  {
    ...
    public:
      ...
      SerializerStatus writeStr(const char* in_string)
      {
        ...
        size_t maxStringSize = getBytesLeft() - sizeof(SizeType);
        size_t size = strnlen(in_string, maxStringSize + 1);
        if (size == maxStringSize + 1) { return error(SerializerStatus::writeStringOutOfRangeOrNotZeroterminated); }
        
        write<SizeType>(size); // we can ignore the return value here, because we already checked for out of range above
      ...
      SerializerStatus writeStr(const char* in_string, SizeType in_size)
      {
        ...
        write<SizeType>(in_size); // we can ignore the return value here, because we already checked for out of range above
  ...
  class Deserializer
  {
    ...
    public:
      ...
      tl::expected<SizeType, DeserializerStatus> readStr(char* out_string, SizeType in_maxStringSize)
      {
        ...
        SizeType size = read<SizeType>().value(); // we can retrieve the value right here, because we already checked for out of range above
        size = size < in_maxStringSize ? size : in_maxStringSize - 1;
